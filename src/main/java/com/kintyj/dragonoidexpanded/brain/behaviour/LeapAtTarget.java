...
/**
 * SmartBrainLib equivalent of
 * {@link net.minecraft.world.entity.ai.goal.LeapAtTargetGoal}
 * <p>
 * Launches the entity at the current attack target and attacks after a
 ...
 * <li>Only leaps if on the ground, has line of sight, and is within 8
 * blocks</li>
 * </ul>
 *
...
public class LeapAtTarget<E extends Mob> extends AnimatableMeleeAttack<E> {
     ...
     *
     * @deprecated use {@link #startCondition(Predicate)} and/or
     *             {@link #leapRange(BiFunction)} instead
    ...
    /**
     * Set the amount that the entity's existing velocity contributes to the jump's
     * velocity.<br>
     * The returned value here acts as a percentage of the entity's existing
     * velocity.<br>
    ...
    protected boolean checkExtraStartConditions(ServerLevel level, E entity) {
        ...
        return entity.onGround() && entity.getSensing().hasLineOfSight(this.target)
                && entity.closerThan(this.target, this.leapRange.apply(entity, this.target));
    }
    ...
    protected void start(E entity) {
        ...
        if (velocity.lengthSqr() > 1.0E-7)
            velocity = velocity.normalize().scale(this.jumpStrength.apply(entity, this.target))
                    .add(entity.getDeltaMovement().scale(this.moveSpeedContribution.apply(entity, this.target)));
